...
router.delete(
  ...
  async (req, res) => {
    try {
      ...
      //adding new friends in ||FRIENDSLIST||
...
router.delete(
  ...
  async (req, res) => {
    try {
      ...
      // Fetch the FriendRequest before deleting
      ...
      // Assuming you have userId and friendId in the request body
      const { userId, friendId } = req.body;

      // Delete the existing FriendRequest
      await FriendRequest.deleteOne({ _id: existingRequest._id });

      // Add new friends in FRIENDSLIST
      const newFriend = await Friends.create({
        userId: userId,
        friendId: friendId,
      });
      
      // Save the newFriend and send a response
      newFriend.save()
        .then((result) => {
          res.status(200).json(result);
