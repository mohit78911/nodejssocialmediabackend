...
router.put("/friendrequest/acceptfriendrequest/:userId", async (req, res) => {
  try {
    ...
    // Logic to accept the friend request and update status in the database
    // ... (your logic to update the friend request status to 'accepted')

    // Assuming you have a User model and 'FriendList' field for storing friends
    const user = await User.findById(userId);
    const friendId = req.body.friendId; // Assuming you receive friendId in the request body

    // Update the user's friend list
    user.friendList.push(friendId);
    await user.save();

    // Update the friend's friend list (assuming you have a similar schema for friends)
