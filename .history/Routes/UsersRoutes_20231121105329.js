...
const storage = multer.diskStorage({
  ...
  filename: function (req, file, cb) {
    cb(null, Date.now() + path.extname(file.or));
  },
});

// File filter function (optional)
const fileFilter = (req, file, cb) => {
  // Check file type, size, or other conditions
  // Example: Allow only jpeg and png files
...
router.post("/upload", upload.single("profilePicture"), (req, res) => {
  ...
  // Process or save the file to a database, etc.
  if (!req.file) {
