...
const verifyToken =  require("../Middlewire/")

router.get("/", verifyToken, async (req, res) => {
  const statusData = await Status.find().populate("userId");
...
router.delete("/delete/:id", verifyToken, async (req, res) => {
  const deleteStatus = await Status.deleteOne({ _id: req.params.id });
  // const updatedStatusList = await deleteStatus.save();
  if (!deleteStatus) {
    return res.status(400).json({ error: "Status can't delete..." });
  }
