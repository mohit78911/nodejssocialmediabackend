...
router.get("/:postId", verifyToken, async (req, res) => {
  ...
  console.log("postId_for_Comments", postId);
  ...
  if (!commentDetails) {
    res.status(400).json({ error: "Comment_Not_Found!" });
    console.log("CommentNotFound!");
  }
  res.send(commentDetails);
  console.log("CommentDataFetch..Done :) ");
});

//Get_Comment_By_Id
router.get("/findbyid/:id",authorization, (req, res) => {
  comments
