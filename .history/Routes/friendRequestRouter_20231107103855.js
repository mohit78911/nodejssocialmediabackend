...
router.put("/friendrequest/:receiver", async (req, res) => {
  try {
    const receiver = req.params.receiver;
    const sender = req.body.sender;
    const action = req.body.action; // New parameter to specify action (accept/reject)

    // Define the newStatus based on the action parameter
    let newStatus;
    if (action === 'accept') {
      newStatus = 'accepted';
    ...
    // Update the document based on the action
    const request = await FriendRequest.updateOne(
      { receiver: receiver },
