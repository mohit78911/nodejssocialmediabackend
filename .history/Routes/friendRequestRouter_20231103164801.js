...
const express = require('express');
const router = express.Router();
const FriendRequest = require('./friendRequest'); // Import your FriendRequest model
...
router.post('/accept-friend-request', async (req, res) => {
  try {
    ...
    const request = await FriendRequest.findByIdAndUpdate(requestId, { status: 'accepted' });
    res.json(request);
  } catch (error) {
...
router.post('/reject-friend-request', async (req, res) => {
  try {
    const { requestId } = req.body;
    const request = await FriendRequest.findByIdAndUpdate(requestId, { status: 'rejected' });
    res.json(request);
  } catch (error) {
