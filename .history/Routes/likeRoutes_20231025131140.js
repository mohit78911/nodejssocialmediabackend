...
router.post("/likepost", async (req, res) => {
  ...
  // const userId = userId;
  const existingLike = await likes.find({ postId: postId });
  if (existingLike) {
    console.log("This User Already Like this Post!");
    res.send("This User Already Like this post!");
    likes.updateOne({_id : req.params.id},{$set:{like : false}}).then((result)=>{res.status(200).send(result)}).catch((error)=>{console.log(error)
    console.log("error With Updating Like")})
    return;
  }
